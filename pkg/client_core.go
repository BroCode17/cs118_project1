...
func Register(client WhatsUpClient, user string) (context.Context, error) {
	...
	tokenResponse, err := client.Connect(context.Background(), &Registration{SourceUser: user})
	if err != nil {
		return nil, errors.New(fmt.Sprintf("unable to connect to server: %s\n", err))
	}

	// Create metadata with the token
	md := metadata.Pairs("token", tokenResponse.Token)
